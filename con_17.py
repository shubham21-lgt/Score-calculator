...
def build_global_attrs(account_details, fact2_enriched):
    ...
    trend_exccc_df = (
        ...
        .withColumn("pct_chg",
                    ...
                    .otherwise(lit(None)))
        .groupBy("cust_id").agg(
            # Exclude month_diff=0 (Java starts at index 1 for comparison)
            fsum(when((col("month_diff") >= 1) & (col("month_diff") <= 24) & (col("pct_chg") >= 0.10),
                      lit(1)).otherwise(lit(0))).alias("totconsinc_bal10_exc_cc_25m"),
            fsum(when((col("month_diff") >= 1) & (col("month_diff") <= 24) & (col("pct_chg") >= 0.05),
