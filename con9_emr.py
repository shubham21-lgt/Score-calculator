...
def load_inputs(trade_csv, account_mapping_csv, product_mapping_csv, bank_mapping_csv):
    """
    Load all four input CSVs and return Spark DataFrames.

    EMR / S3 mode  (_IS_EMR=True):
        Reads directly via spark.read.csv() — S3 paths (s3://...) work natively.
        All type normalisation is done in Spark expressions.
        pandas is NOT used to read files (avoids driver OOM for large trade data).

    Local mode (_IS_EMR=False):
        Uses pandas + _pdf_to_spark() — the Java 25 Hadoop workaround.
        pandas handles type coercion; rows are pushed to Spark via RDD.

    In BOTH modes the returned DataFrames have identical schemas and the
    same downstream pipeline logic runs without any further changes.
    """

    if _IS_EMR:
        # ── EMR / S3 path: native Spark CSV reader ───────────────────────────
        # All columns land as StringType initially (inferSchema=False is safer
        # for large files with mixed nulls). Numeric casting is done in-Spark.

        STR_COLS  = {"account_type_cd", "pay_rating_cd", "account_status_cd",
                     "SUITFILED_WILFULDEFAULT", "SUITFILEDWILLFULDEFAULT",
                     "WRITTEN_OFF_AND_SETTLED", "acct_nb", "resp_code", "closed_dt"}
        INT_COLS  = {"accno", "bureau_mbr_id", "open_dt",
                     "balance_dt", "dayspastdue", "term_freq"}
        DBL_COLS  = {"balance_amt", "credit_lim_amt", "original_loan_amt",
                     "past_due_amt", "actual_pymnt_amt", "written_off_amt",
                     "principal_written_off", "charge_off_amt"}

        def _spark_read(path):
            return (spark.read
                    .option("header", "true")
                    .option("inferSchema", "false")   # all StringType
                    .option("nullValue", "")
                    .csv(path))

        def _cast_trade(df):
            """Cast trade columns to correct types after string read."""
            for c in df.columns:
                if c in INT_COLS:
                    df = df.withColumn(c, F.coalesce(
                        col(c).cast("long"), lit(0).cast("long")))
                elif c in DBL_COLS:
                    df = df.withColumn(c, F.coalesce(
                        col(c).cast("double"), lit(0.0)))
                # STR_COLS and unrecognised cols stay as string
            return df

        # trade_data
        trade_df = _cast_trade(_spark_read(trade_csv))
        if "cons_acct_key" not in trade_df.columns:
            trade_df = trade_df.withColumn("cons_acct_key", col("accno").cast("string"))

        # account_mapping
        acct_map = _spark_read(account_mapping_csv)
        ...
        if "accno" not in acct_map.columns:
            acct_map = acct_map.withColumn("accno", col("cust_id").cast("string"))
        if "relFinCd" not in acct_map.columns:
            acct_map = acct_map.withColumn("relFinCd", lit("1"))
        acct_map = (acct_map
                    .withColumn("cust_id",  col("cust_id").cast("string"))
                    .withColumn("accno",    col("accno").cast("string"))
                    .withColumn("score_dt", col("score_dt").cast("string")))

        # product_mapping — strip b'' wrapping in Spark
        prod_map = _spark_read(product_mapping_csv)
        for _c in ["account_type_cd", "Sec_Uns", "Reg_Com", "Sec_Mov", "Pdt_Cls"]:
            if _c in prod_map.columns:
                prod_map = prod_map.withColumn(
                    _c,
                    F.trim(F.regexp_replace(col(_c).cast("string"), r"^b'|'$", "")))
        prod_map = prod_map.withColumn(
            "account_type_cd",
            F.when(F.regexp_replace(col("account_type_cd"), r"^0+", "") == "",
                   lit("0"))
             .otherwise(F.regexp_replace(col("account_type_cd"), r"^0+", "")))

        # bank_mapping
        bank_map = _spark_read(bank_mapping_csv)

        print(f"  [EMR] trade_data     : {trade_df.count():,} rows")
        print(f"  [EMR] account_mapping: {acct_map.count():,} rows")
        print(f"  [EMR] product_mapping: {prod_map.count():,} rows")
        print(f"  [EMR] bank_mapping   : {bank_map.count():,} rows")

        return trade_df, acct_map, prod_map, bank_map

    else:
        # ── LOCAL path: pandas + RDD (Java 25 workaround) ───────────────────
        ...
        acct_map_pdf = _read(account_mapping_csv, int_cols=["cust_id", "app_date"])
        ...
        def _pdf_to_spark(pdf):
            """Convert pandas DataFrame to Spark via RDD — avoids Arrow on Java 25."""
            ...
            return spark.createDataFrame(
                spark.sparkContext.parallelize(rows), StructType(fields))
...
def dpd_new_udf(dayspastdue, pay_rating_cd, suit_filed, wo_settled, balance_amt):
    ...
    if pr in {"L","B","D"} and dpd == -1:
        return 4
    # FIX 3: pay_rating_cd="S" = Sub-Standard in Indian bureau (NPA stage 1, 91+ DPD equivalent)
    # When dpd is available, bucket is at least 4; when dpd==-1 (not reported), return 4.
    if pr == "S" and dpd == -1:
        return 4
    if pr == "S" and dpd > 0:
        return max(4, (1 if dpd<=30 else 2 if dpd<=60 else 3 if dpd<=90 else
...
def build_global_attrs(account_details, fact2_enriched):
    ...
    accts_opened = (
        ad.groupBy("cust_id").agg(
            ...
            fsum((col("mob") <= 16).cast("int")).alias("nbr_accts_open_l16m"),
    ...
    if _IS_EMR:
        # ── EMR: use pandas_udf (GROUPED_MAP) — fully distributed, no driver collect ──
        from pyspark.sql.functions import pandas_udf, PandasUDFType
        @pandas_udf(emi_schema, PandasUDFType.GROUPED_MAP)
        def _emi_pudf(pdf):
            return compute_emi_udf(pdf)
        account_emi = outflow_input.groupby(["cust_id", "accno"]).apply(_emi_pudf)
        account_emi.cache()
    else:
        # ── LOCAL: rdd.collect() — Java 25 Hadoop workaround ──────────────────
    ...
    if _IS_EMR:
        from pyspark.sql.functions import pandas_udf, PandasUDFType
        @pandas_udf(consec_schema, PandasUDFType.GROUPED_MAP)
        def _consec_pudf(pdf):
            return compute_consec_udf(pdf)
        consec_df = exploded.select("cust_id","idx","dpd").groupby("cust_id").apply(_consec_pudf)
    else:
    ...
    if _IS_EMR:
        from pyspark.sql.functions import pandas_udf, PandasUDFType
        @pandas_udf(_util7m_schema, PandasUDFType.GROUPED_MAP)
        def _util7m_pudf(pdf):
            return compute_util7m_udf(pdf)
        totconsinc_util1_df = _util7m.groupby("cust_id").apply(_util7m_pudf)
    else:
    ...
    if _IS_EMR:
        from pyspark.sql.functions import pandas_udf, PandasUDFType
        @pandas_udf(_cc_bal13_schema, PandasUDFType.GROUPED_MAP)
        def _cc13_pudf(pdf):
            return compute_consinc_bal5_cc(pdf)
        consinc_bal5_cc_df = _cc_bal13.groupby("cust_id").apply(_cc13_pudf)
    else:
    ...
    # ── FIX 5: sum_sanc_amt_uns — sum of max mod_lim across all months per unsec account ──
    sum_sanc_uns_df = (
        exploded
        .filter(col("isUns"))
        .groupBy("cust_id", "accno")
...
# }
    ...
    if _IS_EMR:
        # ── EMR / S3: Spark native write — fully distributed, no driver collect ──
        # output_dir should be an S3 path: s3://bucket/prefix/output/
        # coalesce(1) writes a single part file; remove for large customer sets.
        output_path = output_dir.rstrip("/") + "/scorecard_results"
        (result
         .coalesce(1)
         .write
         .mode("overwrite")
        ...
        # Print sample from Spark (no driver collect needed)
        print("\n--- Sample results ---")
        result.select("cust_id","scorecard_name","final_score").show(20, truncate=False)
        print("\n--- Scorecard distribution ---")
        result.groupBy("scorecard_name").count().orderBy("scorecard_name").show()
    else:
        # ── LOCAL: RDD collect → pandas → CSV (Java 25 workaround) ─────────
...
if __name__ == "__main__":
    ...
    # ── Path resolution ───────────────────────────────────────────────────────
    # On EMR: pass S3 paths as CLI args:
    #   spark-submit con9_emr.py \
    #     s3://bucket/input/trade_data.csv \
    #     s3://bucket/input/account_mapping.csv \
    #     s3://bucket/input/product_mapping.csv \
    #     s3://bucket/input/bank_mapping.csv \
    #     s3://bucket/output/
    # Or set env vars: SCORECARD_INPUT_DIR, SCORECARD_OUTPUT_DIR
    # Set EMR_MODE=1 in your EMR step environment.
    #
    # Locally: pass local paths or rely on SCORECARD_INPUT_DIR env var.
    _default_base = os.environ.get("SCORECARD_INPUT_DIR", os.getcwd())
    _default_out  = os.environ.get("SCORECARD_OUTPUT_DIR", os.getcwd())

    TRADE_CSV    = sys.argv[1] if len(sys.argv) > 1 else os.path.join(_default_base, "trade_data.csv")
    ACCT_MAP_CSV = sys.argv[2] if len(sys.argv) > 2 else os.path.join(_default_base, "account_mapping.csv")
    PROD_MAP_CSV = sys.argv[3] if len(sys.argv) > 3 else os.path.join(_default_base, "product_mapping.csv")
    BANK_MAP_CSV = sys.argv[4] if len(sys.argv) > 4 else os.path.join(_default_base, "bank_mapping.csv")
    OUTPUT_DIR   = sys.argv[5] if len(sys.argv) > 5 else _default_out

    if not _IS_EMR:
        # os.path.exists() only works for local paths — skip on EMR (S3 paths)
        for path, label in [
