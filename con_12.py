...
def build_global_attrs(account_details, fact2_enriched):
    ...
    acct_counts = (
        ad.groupBy("cust_id").agg(
            ...
            # FIX A (trade-data confirmed): avg_mob_reg_36 excludes LAP accounts only.
            # Real data: cust 10003821 has type 195 (LAP) with mob=100.
            # avg([20,39,100,109,130])=79.6 (code) vs GT=74.5=avg([20,39,109,130]).
            # Type 195 is LAP (Loan Against Property) — excluded from avg_mob_reg_36.
            # isLapHl includes BOTH HL (type 58) AND LAP (type 195).
            # But excluding ALL isLapHl removes HL which breaks many customers.
            # The correct exclusion: only isPL & isSec (= LAP in Indian bureau coding).
            # LAP accounts: type 195 → isPL=True, isSec=True (Pdt_Cls=RegSec or SecMov).
            # HL accounts:  type 58  → isHL=True, isSec=True (should stay in avg).
            # FIX: exclude only accounts that are PL AND Secured (=LAP, not HL).
            favg(when(~(col("isPL") & col("isSec")), col("mob"))).alias("avg_mob_reg_36"),
    ...
    # totconsdec — based on ALL accounts (FIX B confirmed by trade data)
    trend_all_df = (
        monthly_bal_all
        ...
        .withColumn("pct_chg_all",
                    ...
                    .otherwise(lit(None)))
        .groupBy("cust_id").agg(
            # Full 1..35 window for totconsdec
            fsum(when((col("month_diff") >= 1) & (col("month_diff") <= 35) & (col("pct_chg_all") < 0),
    ...
    def compute_consinc_bal10_exc_cc(pdf):
        ...
        for cust_id, grp in pdf.groupby("cust_id"):
            ...
            # Build dict: month_diff → pct_chg (ASC order, lead already applied)
            pct_map = dict(zip(grp["month_diff"], grp["pct_chg"]))
            consec = 0
            for m in range(1, 25):   # months 1..24 from most recent
                pct = pct_map.get(m)
    ...
    # consinc_bal10_exc_cc_25m consecutive UDF — uses excCC balance trend
    _trend_pct_rows = (
        monthly_bal_exccc
        .withColumn("prev_bal", lead("tot_bal").over(w_trend))
        .withColumn("pct_chg",
                    when(col("prev_bal") > 0,
                         F.try_divide(col("tot_bal") - col("prev_bal"), col("prev_bal")))
                    .otherwise(lit(None)))
        .filter((col("month_diff") >= 1) & (col("month_diff") <= 24))
        .select("cust_id", "month_diff", "pct_chg")
    )
    _consec_rows = _trend_pct_rows.rdd.collect()
    _consec_pdf2  = pd.DataFrame(_consec_rows, columns=["cust_id","month_diff","pct_chg"])
    ...
    # FIX 14: Lowered balance_amt filter from > 500 to > 0.
    # The > 500 threshold excluded valid EMI-paying accounts with small balances
    # (e.g. near-end-of-loan accounts with balance 100-500 still make EMI payments).
    # org_car uses balance > 0 as the signal for an active account with payments.
    # Also removed the isLiveAccount filter — closed accounts in last N months
    # still have balance history used for EMI estimation.
    outflow_input = (
        fe.select("cust_id","accno","cons_acct_key","month_diff","balance_amt","isCC","isPlCdTw","isSecMovRegSec","isRegUns")
        .filter(col("balance_amt") > 0)
        .join(
    ...
    bal_ratio_df = (
        live_bal_all.groupBy("cust_id").agg(
            ...
            # FIX: idx 7..12 (was 6..11) to match the 7-12m month window
    ...
    # --- nbr_0_24m_live -----------------------------------------------------
    # Java: count month-rows on LIVE accounts where any DPD > 0 in last 24m.
    # FIX 15: Two changes:
    #   (a) Use raw_dpd > 0 instead of dpd > 0 so events masked by bal<=500 are caught.
    #   (b) Return 0 (not NaN) when a customer has live accounts but no DPD events
    #       (org_car returns 0 for clean customers, not NaN).
    ...
    # Count of all live-account month-rows in window (for the zero-fill denominator)
    ...
    nbr_0_24m_live_df = (
        ...
        .agg(
            fsum(when(
                (col("dpd") > 0) | (col("raw_dpd") > 0), lit(1)
            ).otherwise(lit(0))).alias("nbr_0_24m_live")
        )
...
# ## Function Status — What Was Changed vs Untouched

# ### ✅ MODIFIED (we changed these)

# | Function | # Fixes | What changed |
# |---|---|---|
# | `load_inputs()` | 3 | Dedup trade data on `(cons_acct_key, balance_dt)` + auto-detect CSV/Excel |
# | `build_fact2_enriched()` | 2 | Added `raw_dpd` to monthly struct + `is_not_closed` per month |
# | `build_account_details()` | 5 | `fmax(open_dt)` for MOB + `fmax()` for all 20 boolean flags + removed duplicate define |
# | **`build_global_attrs()`** | **23** | The biggest function — all scorecard variable fixes (see below) |
# | `compute_trigger_eligibility()` | 1 | `raw_dpd` column referenced for eligibility |
# | `compute_final_score()` | 1 | Column dedup before final select |
# | `run_pipeline()` | 3 | Datetime timestamp + column dedup + memory settings |
# | `dpd_new_udf()` | 3 | WO_SETTLED sentinel + `pay_rating S=0` + `balance<=500` guard |
# | `derog_flag_udf()` | 1 | `pay_rating '000'` no longer triggers derog |
# | `add_product_flags()` | 2 | `isLapHl` corrected for LAP exclusion in `avg_mob_reg_36` |

# ### 🔵 UNTOUCHED (correct as-is, no changes needed)

# | Function | Why untouched |
# |---|---|
# | `build_fact2()` | Join logic correct — no issues found |
# | `modified_limit_udf()` | `max(balance, credit_lim, original_loan)` — correct |
# | `apply_score_trigger()` | ST2/ST3 eligibility logic correct |
# | `clean_numeric_date()` | Trivial utility |

# ### ⚠️ STILL BROKEN (not touched, still needs fixing)

# | Function | Variables affected | What's wrong |
# |---|---|---|
# | `compute_emi_udf()` | `HL_LAP_outflow`, `AL_TW_outflow`, `PL_outflow`, `CD_outflow` | EMI direction reversed — need Java spec |
# | `avg_gap_udf()` | `freq_between_accts_all`, `freq_between_accts_unsec_wo_cc`, `freq_between_installment_trades` | Formula: code uses `avg(gap)`, GT likely uses `avg(mob)` |
# | `compute_consinc_bal5_cc()` | `consinc_bal5_cc_13m` | 5% threshold direction reversed — `newer > older` not `older > newer` |

# ---

# ## Variables Still Needing Code Fixes (inside `build_global_attrs`)

# | Variable | Problem | What you need |
# |---|---|---|
# | `mon_since_max_bal_l24m_uns` | Off by +1 always | Use `fmax(idx)` where `bal == max_bal` instead of `fargmax` |
# | `util_l12m_uns_tot` | Returns ~half of GT | Check `idx<=23` vs `idx<=11` — window may be 24m instead of 12m |
# | `mon_sin_recent_1 / mon_sin_first_1` | Returns 35 when GT=NaN | Remove `.otherwise(lit(35))` — let `fmin(when(...))` return null naturally |
# | `open_cnt_0_6_by_7_12` | Returns NaN when GT=0 | Use `coalesce(result, lit(0))` when 7-12m accounts exist |
# | `max_dpd__UNS__L6M / L_6_12_M` | 67/60 Code=NaN | Apply `_effective_dpd()` to these two variables (already done for L36M) |
# | `nbr_cc40l6m_tot_accts_36` | 6x Ref=NaN | Remove the `join(acct_counts, "right")` — only fillna(0) for CC customers |
# | `nbr_cl_tot_accts_36` / `nbr_pl_tot_accts_36` | 30/17x Code=NaN | `CL_CODES` / `PL_CODES` incomplete — need full list from bureau |
# | `nbr_comsec_tot_accts_36` | 20x Code=NaN | `COM_SEC_CODES` incomplete — need bureau mapping |