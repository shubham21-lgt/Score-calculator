...
# ══════════════════════════════════════════════════════════════════════════
    ...
    _freq_pdf = pd.DataFrame(_freq_rows, columns=["cust_id", "cons_acct_key", "tc", "open_abs", "isUns", "isCC"])
    ...
    def _compute_freq(cust_pdf):
        ...
        try:
            ...
            cust_pdf = cust_pdf.dropna(subset=["open_abs"])
            # FIX: custom_var L401 sorts by min_open_dt only, with stable order.
            # Previous code sorted by (open_abs, _tc_num) which put type 5 (CC_EXCL)
            # BEFORE type 189 (qualifying) when they had same open date, causing
            # gap=0 instead of the real gap to the qualifying account.
            # Use cons_acct_key as deterministic tie-breaker (matches typical CSV order).
            cust_pdf = cust_pdf.sort_values(
                ["open_abs", "cons_acct_key"], kind="mergesort").reset_index(drop=True)
        except Exception:
