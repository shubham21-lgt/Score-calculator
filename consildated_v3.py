...
#
#   account_mapping : cust_id ↔ accno + relFinCd (ownership)
#                     Columns: accno, cust_id, score_dt, relFinCd  [+ others]
#
#   product_mapping : account_type_cd → Sec_Uns, Reg_Com, Sec_Mov, Pdt_Cls
#   bank_mapping    : bureau_mbr_id   → Category (PSB/PVT/NBF/COB/SFB)
#
# PIPELINE PHASES
# ─────────────────────────────────────────────────────────────────────────────
...
def load_inputs(trade_csv, account_mapping_csv, product_mapping_csv, bank_mapping_csv):
    ...
    trade_str  = ["account_type_cd", "pay_rating_cd", "account_status_cd",
                  ...
                  "WRITTEN_OFF_AND_SETTLED", "acct_nb", "resp_code"]
    trade_int  = ["accno", "bureau_mbr_id", "open_dt", "closed_dt",
    ...
    prod_pdf["account_type_cd"] = prod_pdf["account_type_cd"].astype(str).str.strip()
...
def build_global_attrs(account_details, fact2_enriched):
    ...
    accts_opened = (
        ad.groupBy("cust_id").agg(
            ...
            fmin("mob").alias("mon_since_last_acct_open"),   # min mob = most recently opened
        )
    )
    ...
    outflow_input = (
        fe.select("cust_id","accno","month_diff","balance_amt","isCC","isPlCdTw","isSecMovRegSec","isRegUns")
        .filter(col("balance_amt") > 500)
        .join(
            account_details.select("cust_id","accno",
    ...
    cc40_df = (
        ...
        .groupBy("cust_id")
        .agg(count("accno").alias("nbr_cc4016m_tot_accts_36"))
    )
    ...
    cc40_6m_df = (
        ...
        .groupBy("cust_id")
        .agg(count("accno").alias("nbr_cc40l6m_tot_accts_36"))
    )
    ...
    nbr_0_24m_live_df = (
        # B5 FIX: count total account-month occurrences with DPD>0 in 24m
        # (not distinct accounts — reference value ~2x suggests it counts months)
        exploded
        .filter((col("idx") < 24) & (col("dpd") > 0))
        .join(ad.select("cust_id","accno","isLiveAccount"), ["cust_id","accno"], "left")
        .filter(col("isLiveAccount") == True)
        .groupBy("cust_id")
        .agg(F.count("accno").alias("nbr_0_24m_live"))
    )
    ...
    latest_acct_type_df = (
        ad.select("cust_id","open_dt","account_type_cd","isSec","isCC","isPL","isTW")
        .withColumn("_rank", F.row_number().over(
            Window.partitionBy("cust_id").orderBy(col("open_dt").desc())))
        .filter(col("_rank") == 1)
...
def compute_final_score(global_attrs_with_trigger):
        ...
        for _, row in pdf.iterrows():
            ...
            for attr, w in coeff.items():
                ...
                if val is not None and not (isinstance(val, float) and np.isnan(val)):
                    effective = 0.0 if float(val) == -999.0 else float(val)
                    contrib   = effective * w
                    score    += contrib
    ...
    # RDD collect → pandas groupby → RDD parallelize (zero Arrow path)
    _score_cols = ga.columns
    _score_rows = ga.rdd.collect()
    _score_pdf  = pd.DataFrame(_score_rows, columns=_score_cols)
    _score_parts = [score_udf(grp) for _, grp in _score_pdf.groupby("cust_id")]
    _score_result = pd.concat(_score_parts, ignore_index=True) if _score_parts else pd.DataFrame(
        columns=["cust_id","final_score","score_breakdown"])
    scores = spark.createDataFrame(
        spark.sparkContext.parallelize([tuple(r) for r in _score_result.itertuples(index=False)]),
        schema=score_schema)
...
# }
    ...
    flat_csv = os.path.join(output_dir, "scorecard_results_v3.csv")
