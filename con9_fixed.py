...
def build_global_attrs(account_details, fact2_enriched):
    ...
    util_all_df = (
        ...
        .groupBy("cust_id")
        .agg(
            # Mean of per-row ratios (your ground truth’s avg_*):
            favg(when(col("idx") <= 2,  col("util"))).alias("avg_util_l3m_all_tot"),
            favg(when(col("idx") <= 5,  col("util"))).alias("avg_util_l6m_all_tot"),
            favg(when(col("idx") <= 11, col("util"))).alias("avg_util_l12m_all_tot"),
    ...
    # ── util_uns_df: isUns & isLive (NO ~isCC filter)
    # ROOT CAUSE FIX: type-5 is flagged BOTH isCC AND isUns.
    # Old ~isCC filter removed ALL UnSec rows for customers whose only UnSec product
    # is type-5, producing NaN. Confirmed fixes:
    #   util_l6m_uns_tot  = favg(bal/lim) on isUns & isLive, idx<=5  -> 0.044425
    #   util_l12m_uns_tot = favg(bal/lim) on isUns & isLive, idx<=11 -> 0.045911
    util_uns_df = (
        exploded
    ...
    # sum_sanc_amt_uns — sum of max mod_lim per LIVE unsec account
    # org_car: sum_sanc_amt_uns uses cond['live'] — closed accounts excluded
    sum_sanc_uns_df = (
        exploded
        .join(_live_for_util, ["cust_id", "accno"], "inner")  # isLive filter
        .filter(col("isUns"))
        .groupBy("cust_id", "accno")
...
# dpd_new_udf — pay_rating='S' now returns 0 (was 4)
# add_product_flags — isSecMov flag correctly wired for max_sanc_amt_secmov
# build_account_details (Phase 3) — isLiveAccount split into _has_real_close + _has_open_status
# build_global_attrs (Phase 4) — 11 separate fixes across util, DPD, nbr counts, EMI, sanction amounts
# run_pipeline (Phase 6) — output_cols and rename_map updated for new columns

# Functions not touched (7):

# load_inputs, build_fact2 — data loading/joining, no logic errors
# build_fact2_enriched (Phase 2) — unchanged but automatically inherits the dpd_new_udf fix
# compute_trigger_eligibility (Phase 5A) — scorecard trigger logic unchanged
# apply_score_trigger (Phase 5B) — scorecard selection unchanged
# compute_final_score (Phase 5C) — final scoring unchanged
# modified_limit_udf, derog_flag_udf — no issues found