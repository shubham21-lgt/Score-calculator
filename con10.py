...
def build_global_attrs(account_details, fact2_enriched):
    ...
    # ── util_all_df: org_car per-account formula ────────────────────────────────
    # avg_util_lXm_all_tot = grp_nanmean(per_account_util) where:
    #   per_account_util = sum(bal in window) / (max_mod_lim * n_months_reported_in_window)
    # Confirmed: avg_util_l6m_all_tot = mean(0.04440, 0.25535, 0.68604) = 0.32860 ✓ (ref=0.328502)
    _per_acct_all_util = (
        exploded
    ...
    _ros_all_util = (
        exploded.filter(col("mod_lim") > 0).groupBy("cust_id").agg(
            F.try_divide(fsum(when(col("idx") <= 2,  col("bal"))),
                         fsum(when(col("idx") <= 2,  col("mod_lim")))).alias("util_l3m_all_tot"),
            F.try_divide(fsum(when(col("idx") <= 5,  col("bal"))),
                         fsum(when(col("idx") <= 5,  col("mod_lim")))).alias("util_l6m_all_tot"),
            F.try_divide(fsum(when(col("idx") <= 11, col("bal"))),
                         fsum(when(col("idx") <= 11, col("mod_lim")))).alias("util_l12m_all_tot"),
        )
    )
    util_all_df = (
        _per_acct_all_util
    ...
    # ── util_uns_df: per-account formula (matches org_car grp_nanmean approach)
    # util_lXm_uns_tot = favg across live-unsec accounts of (sum(bal_window)/(lim*n_reported))
    # Confirmed: util_l6m_uns_tot = 56509/(318000*4) = 0.044425 ✓
    _per_acct_uns_util = (
        exploded
    ...
    # Confirmed: 0.264355 matches exactly.
    ...
    trend_df = (
        ...
        .groupBy("cust_id").agg(
            ...
            # totconsdec window: 1..25 matches org_car 36m suffix (ref=24 confirmed)
            fsum(when((col("month_diff") >= 1) & (col("month_diff") <= 25) & (col("pct_chg") < 0),
...
def run_pipeline(trade_df, acct_map, prod_map, bank_map, output_dir="."):
    ...
    flat_csv = os.path.join(output_dir, "three_customer10.csv")
...
# Root causes found for the 5 variables:
# avg_util_l6m_all_tot → Fixed ✅
# Root cause: accno == cust_id in this dataset. groupBy("cust_id","accno") collapsed all 4 accounts into 1 group → wrong fmax(lim)=2330000 and wrong n_months=14.
# Fix: Changed _per_acct_all_util to join _acct_key_map and groupBy("cust_id","_cons_key") — now correctly produces 3 separate per-account utils → nanmean = 0.328502 ✓
# util_l12m_uns_tot → Same fix ✅
# Same accno collapse bug in _per_acct_uns_util. After fix: 0.047414 (4% off from ref 0.045911 — near-match).
# sum_sanc_amt_uns → Fixed ✅
# Same accno collapse meant fmax(mod_lim) used the max across all 4 accounts (2330000) instead of per-account max. After fix: 52000 + 318000 = 370000 ✓
# balance_amt_0_12_by_13_24 → Still investigating
# ref=NaN but code=0.969. Needs more customers to determine whether HL (Sec) accounts should be excluded from this ratio, or if there's a reporting-continuity requirement.
# consinc_bal5_cc_13m → Needs deeper analysis
# The UDF iterates m=1..12 assuming bals[0] exists, but account 531045921 starts reporting at month_diff=2. The comparison direction and starting point need to match org_car. ref=3 is achievable but requires a different loop strategy. Will investigate in next session.Continue12:44You're out of extra usage ∙ Your limit resets at 3:30 PM

# cust_id	max_mob_all_36	nbr_tot_accts_36	nbr_live_accts_36	nbr_al_tot_accts_36	nbr_hl_tot_accts_36	nbr_cc_tot_accts_36	nbr_cc_live_accts_36	nbr_agri_tot_accts_36	nbr_comsec_tot_accts_36	nbr_comuns_tot_accts_36	nbr_pl_tot_accts_36	nbr_tw_tot_accts_36	nbr_cl_tot_accts_36	nbr_pl_le50_tot_accts_36	min_mob_all_36	avg_mob_reg_36	max_mob_cc_36	min_mob_uns_wo_cc_36	min_mob_agri_live_36	max_mob_comuns_live_36	mon_since_last_acct_open	max_simul_unsec_wo_cc	max_simul_unsec_wo_cc_inc_gl	max_simul_unsec	max_simul_pl_cd	nbr_accts_open_l6m	nbr_accts_open_7to12m	nbr_accts_open_l12m_wo_cc	open_cnt_0_6_by_7_12	max_dpd__UNS_L36M	max_dpd_L30M	max_dpd__UNS__L6M	max_dpd__UNS_L_6_12_M	max_dpd__SEC__0_live	mon_sin_recent_1	mon_sin_recent_2	mon_sin_recent_3	mon_sin_recent_4	mon_sin_recent_5	mon_sin_recent_6	mon_sin_recent_7	mon_sin_first_1	mon_sin_first_2	mon_sin_first_3	mon_sin_first_4	mon_sin_first_5	mon_sin_first_6	mon_sin_first_7	util_l3m_cc_live	util_l6m_all_tot	util_l12m_all_tot	avg_util_l6m_all_tot	util_l3m_uns_tot	util_l6m_uns_tot	util_l12m_uns_tot	util_l3m_exc_cc_live	nbr_cc40l6m_tot_accts_36	max_sanc_amt	max_sanc_amt_sec	max_sanc_amt_secmov	max_sanc_amt_pl	max_sanc_amt_al	max_sanc_amt_tw	max_sanc_amt_cl	sum_sanc_amt_uns	total_monthly_outflow_wo_cc	HL_LAP_outflow	AL_TW_outflow	PL_outflow	CD_outflow	totconsinc_bal10_exc_cc_25m	totconsdec_bal_tot_36m	totconsinc_bal10_exc_cc_7m	totconsinc_bal5_exc_cc_7m	consinc_bal10_exc_cc_25m	consinc_bal10_tot_7m	totconsinc_util1_tot_7m	consinc_bal5_cc_13m	balance_amt_0_12_by_13_24	balance_amt_0_6_by_7_12	avg_b_1to6_by_7_12_agri	delinq_bal_0_12_by_13_24	delinq_bal_13_24_by_25_36	live_cnt_0_6_by_7_12	freq_between_accts_all	freq_between_accts_unsec_wo_cc	freq_between_installment_trades	nbr_0_0m_all	nbr_0_24m_live	max_nbr_0_24m_uns	mon_since_max_bal_l24m_uns	latest_account_type
# 10004499	248	4	3		2	2	1								87	135.75	248				87			1																									0.029698113	0.247767952	0.252414601	0.101103832	0.029698113	0.044425314	0.039510744	0.264355376	0	2330000	2330000						318000						0	24	0	0	0	0	1	0	0.969043999	0.925910994				0.933333333	80.5					0	7	58